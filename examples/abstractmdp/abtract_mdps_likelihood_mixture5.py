"""
Train encoder only using diagonal
"""
import numpy as np
import gym
import numpy as np
...
class AbstractMDPsContrastive:
    def __init__(self, envs):
        ...
        for j in range(self.n_abstract_mdps):
            ...
            encoder = CNN(self.state_shape[0], self.state_shape[1], 3, self.abstract_dim,
                          [3, 3], [8, 8], [1, 1], [1, 1], hidden_sizes=(32, 32), output_activation=nn.Softmax(),
                          layernorm=True)
    ...
    def train(self, max_epochs=100):
        ...
        a1 = np.array([ [0, 0.5, 0.5, 0],
                        ...
                        [0.5, 0, 0, 0.5],
                        [0, 0.5, 0.5, 0]])
        a2 = np.array([ [0, 0.9, 0.05, 0.05],
                        [0.9, 0, 0.05, 0.05],
                        [0.05, 0.05, 0, 0.9],
                        [0.05, 0.05, 0.9, 0]])
        all_abstract_t = from_numpy(np.stack([a1, a2]))
    ...
    def compute_empirical_t(self, env, encoder, hardcounts=False):
        ...
        self_e_t = (a_t / (a_t.sum(1, keepdim=True) + 1e-12))[np.arange(self.abstract_dim), np.arange(self.abstract_dim)]

        a_t[np.arange(self.abstract_dim), np.arange(self.abstract_dim)] = 0
        for i in range(self.abstract_dim):
            n_a_t[i, :] += (a_t[i, :] + 1e-8) / (a_t[i, :] + 1e-8).sum()

        return n_a_t, self_e_t
    ...
    def train_epoch(self, epoch, mixture, all_abstract_t, self_t):
        ...
        with torch.no_grad():
            ...
            mixture[:] = all_concrete_ll / (all_concrete_ll.sum(1, keepdim=True) + 1e-12) #p(m|h)
            mixture_inv = mixture / mixture.sum(0, keepdim=True) # p(h|m)
            #import pdb; pdb.set_trace()
            # compute abstract transitions using mixture
            all_empirical_t = [] #(n_abstract_mdps, n_envs, abstract_dim, abstrat_dim)
            all_e_t = []
            for i in range(self.n_abstract_mdps):
                empirical_t, self_e_t = zip(*[self.compute_empirical_t(self.envs[j], self.all_encoder_lst[i], hardcounts=False) for j in range(self.n_envs)])
                all_empirical_t.append(empirical_t)
                all_e_t.append(self_e_t)
        ...
        # Compute self transitions
            ...
            #    e_t = torch.stack([all_e_t[j][i] for j in range(self.n_abstract_mdps)])
            #    self_t[i, :] = (mixture[i, :].view(-1, 1) * e_t).sum(0)
    ...
    def train_encoder(self, epoch, mixture, all_abstract_t, self_t):
        ...
        def compute_abstract_trans_ll(y1, y2, A, self_t):
            ...
            a_ll = from_numpy(np.zeros(1))  # likelihood for mixture
            loss_ll = from_numpy(np.zeros(1))
            for i in range(self.abstract_dim):
                for j in range(self.abstract_dim):
                    if i == j:
                        loss_ll += (y1[:, i] * y2[:, j] * torch.log(self_t[i].detach() + 1e-12)).sum()
                    else:
                        # ((1-self_t[i].detach())
                        a_ll += (y1[:, i] * y2[:, j] * torch.log( A_nondiag[i, j].detach() + 1e-12)).sum()
                        loss_ll += (y1[:, i] * y2[:, j] * torch.log((1-self_t[i].detach()) * A_nondiag[i, j].detach() + 1e-12)).sum()
        ...
        for i in range(self.n_envs):
            for j in range(self.n_abstract_mdps):
                ...
                concrete_ll, loss_ll = compute_abstract_trans_ll(y1, y2, all_abstract_t[j], self_t[i])
...
if __name__ == '__main__':
    ...
    envs = [FourRoomsModEnv(gridsize=15, room_wh=(7, 7)),
            ...
            #FourRoomsModEnv(gridsize=15, room_wh=(7, 6), close_doors=["north", "south"])
