...
class AbstractMDPsContrastive:
    ...
    def train_epoch(self, epoch, mixture, all_abstract_t):
        ...
        mixture[:] = all_concrete_ll / (all_concrete_ll.sum(1, keepdim=True) + 1e-8)
        ...
        all_abstract_t[:] = all_abstract_t / all_abstract_t.sum(1, keepdim=True)
    ...
    def compute_abstract_trans_ll(self, y1, y2, A):
        ...
        for i in range(self.abstract_dim):
            for j in range(self.abstract_dim):
                a_ll += (y1[:, i] * y2[:, j] * torch.log(A[i, j].detach() + 1e-8)).sum()
